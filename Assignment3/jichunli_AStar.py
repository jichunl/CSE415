...
AStar Search of a problem space.
 ...
 Jichun Li, Univ. of Washington.
 Paul G. Allen School of Computer Science and Engineering

 Usage:
 python3 jichunli_AStar.py FranceWithCosts
...
if sys.argv == [''] or len(sys.argv) < 2:
    ...
    # import jichunli_EightPuzzleWithHamming as Problem
    import jichunli_EightPuzzleWithManhattan as Problem
else:
    import importlib
...
def AStar(initial_state):
    ...
    OPEN.insert(initial_state, h_value(initial_state))
    ...
    while len(OPEN) > 0:
        ...
        suc_cand = [] # list of candidates of successors
        for op in Problem.OPERATORS:
            if op.precond(S):
                new_state = op.state_transf(S)
                if new_state not in CLOSED:
                    suc_cand.append(new_state)

        for candidate in suc_cand:
            edge_cost = S.edge_distance(candidate)
            new_g = gs + edge_cost
            new_f = new_g + h_value(candidate)
            if candidate in OPEN:
                current_f = OPEN[candidate]
                if new_f < current_f:
                    del OPEN[candidate]
                    OPEN.insert(candidate, new_f)
                    BACKLINKS[candidate] = S
                    g[candidate] = new_g
            else:
                g[candidate] = new_g
                BACKLINKS[candidate] = S
